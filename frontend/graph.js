...
window.schemaData = schemaData; // Make accessible globally for schema_visualizer.js
let simulation = null;
...
document.addEventListener('DOMContentLoaded', () => {
    initializeGraph();
    if (window.schemaVisualizer && window.schemaVisualizer.initializeSchema) {
        window.schemaVisualizer.initializeSchema();
    }
    setupEventListeners();
    loadGraph();
    loadSchema();
...
function setupEventListeners() {
    ...
    if (schemaShowFK) {
        schemaShowFK.addEventListener('change', () => {
            if (window.schemaVisualizer && window.schemaVisualizer.renderSchema) {
                window.schemaVisualizer.renderSchema();
            }
        });
    }
    if (schemaShowInferred) {
        schemaShowInferred.addEventListener('change', () => {
            if (window.schemaVisualizer && window.schemaVisualizer.renderSchema) {
                window.schemaVisualizer.renderSchema();
            }
        });
    ...
    if (schemaReset) {
        schemaReset.addEventListener('click', () => {
            if (window.schemaVisualizer && window.schemaVisualizer.resetSchemaZoom) {
                window.schemaVisualizer.resetSchemaZoom();
            }
        });
    }
...
async function loadSchema() {
    try {
        ...
        window.schemaData = schemaData; // Update global reference
        if (window.schemaVisualizer && window.schemaVisualizer.renderSchema) {
            window.schemaVisualizer.renderSchema();
        }
    } catch (error) {
...
// Schema functions moved to schema_visualizer.js
...
// Schema rendering moved to schema_visualizer.js
